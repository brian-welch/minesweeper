...
if (difficultyLevel() < 1 || !difficultyLevel()) {
  window.location.search = "?rows=10&columns=10&level=3&leve_must_be_greater_than_1";
}
...
function extract(word) {
  let x = [];
  word.split(' ').join('').split('').forEach(function(letter, index){
    if ((index + 1) % 2 == 0) { x.push(letter) };
  });
  return x.join('');
};
...
const showDeadMessage = () => {
  ...
  messageBox.innerHTML = `<input type="button" value="Play Again" id="restartButton">`;
  document.querySelector("#restartButton").addEventListener("click", () => {
    window.location.reload(true);
  });
};
